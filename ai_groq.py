...
if not GROQ_KEY:
    raise ValueError("Chave GROQ_KEY não encontrada nas variáveis de ambiente.")
...
def gerar_resposta_ia(prompt: str, max_tokens: int = 300) -> str:
    ...
    try:
        ...
        # Retorna conteúdo da primeira escolha
        return response.choices[0].message["content"]
    except Exception as e:
        # Log do erro e fallback
        print(f"[ai_groq] Erro ao gerar resposta: {e}")
        return "Erro ao gerar resposta IA. Por favor, tente novamente mais tarde."

# Função helper compatível com o back-end
def analisar_com_groq(texto: str, key: str = None):
    """
    Função de interface para o back-end.
    Usa a chave fornecida ou a variável de ambiente.
    """
    global client
    if key:
        client = Groq(api_key=key)
    return "Produtivo", gerar_resposta_ia(texto)
